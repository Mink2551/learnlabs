...
function Navbar() {
  ...
  const Sidebar = (): JSX.Element => {
    return (
      <div className={`fixed top-0 left-0 h-screen w-80 bg-gray-800 text-white transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out z-[9999]`}>
        ...
        <div className='ml-4 grid gap-10'>
          ...
          <Link href='/test' className='hover:text-SC4 text-white duration-100 flex px-3 py-2 rounded-md'>
            <GrDocumentTest className='items-center mt-1 flex text-xl mr-2'/>
            <h2 className='font-medium text-xl'>TestLabs</h2>
          </Link>
          <Link href='/docs' className='hover:text-SC4 text-white duration-100 flex px-3 py-2 rounded-md'>
